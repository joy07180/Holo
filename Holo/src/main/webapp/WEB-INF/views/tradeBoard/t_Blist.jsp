...
&nbsp;&nbsp;<a href="binsertf">새글등록</a>
			...
			<div id="searchBar">
				<form action="t_bcrilist" method="get">
					...
					<c:forEach var="trade" items="${pageMaker.cri.check}">
						<c:if test="${trade=='sell'}">
							<input type="checkbox" name="check" value="sell" checked
								class="clear">판매&nbsp;
				<c:set var="ckPrint" value="true" />
					...
					<c:forEach var="trade" items="${pageMaker.cri.check}">
						<c:if test="${trade=='buy'}">
							<input type="checkbox" name="check" value="buy" checked
								class="clear">구매&nbsp;
				<c:set var="ckPrint" value="true" />
						</c:if>
					</c:forEach>
			...
			<table width=100%>
				...
				<c:if test="${loginID!='admin'}">
				    ${trade_board.id}
							...
							<td>${trade_board.regdate}</td>
							<td>${trade_board.cnt}</td>
						</tr>
					</c:forEach>
				</c:if>
			</table>
